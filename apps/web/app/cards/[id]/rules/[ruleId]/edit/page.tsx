...
export default function EditRulePage() {
  ...
  function onSubmit(e: React.FormEvent) {
    ...
    if (form.rewardType === 'miles' && form.milesBlockSize !== undefined && (!isFinite(form.milesBlockSize) || form.milesBlockSize <= 0)) {
      setMessage('Block size must be a positive whole number');
      return;
    }
    if (form.startDate && form.endDate) {
      const start = new Date(form.startDate);
      const end = new Date(form.endDate);
      if (start > end) { setMessage('End date must be on or after start date'); return; }
    }
          ...
          <form className="space-y-4" onSubmit={onSubmit}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              <div>
                ...
                <select
                  ...
                  onChange={e => {
                    const value = e.target.value as 'cashback' | 'miles';
              ...
              <div>
                ...
                <input type="number" step="0.01" min={0.01} className="w-full px-3 py-2 border rounded" value={form.rewardValue} onChange={e => setForm({ ...form, rewardValue: Number(e.target.value) })} />
                <p className="text-xs text-muted-foreground mt-1">% for cashback; miles per dollar for miles</p>
              </div>
              {form.rewardType === 'miles' && (
                <div>
                  <label className="block text-sm mb-1">Block size ($)</label>
                  <input type="number" step="1" min={1} className="w-full px-3 py-2 border rounded" value={form.milesBlockSize || ''} onChange={e => setForm({ ...form, milesBlockSize: e.target.value ? Number(e.target.value) : undefined })} />
                </div>
              )}
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <div>
                ...
                <input type="number" step="0.01" min={0} className="w-full px-3 py-2 border rounded" value={form.minimumSpend ?? ''} onChange={e => setForm({ ...form, minimumSpend: e.target.value ? Number(e.target.value) : undefined })} />
              </div>
              <div>
                <label className="block text-sm mb-1">Maximum spend ($)</label>
                <input type="number" step="0.01" min={0.01} className="w-full px-3 py-2 border rounded" value={form.maximumSpend ?? ''} onChange={e => setForm({ ...form, maximumSpend: e.target.value ? Number(e.target.value) : undefined })} />
              </div>
            </div>
